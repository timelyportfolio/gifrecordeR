...
/*
  ...
  Copyright (c) 2014 Niklas von Hertzen (@niklasvh)
  ...
  return function(zStack, options, doc, queue, _html2canvas) {
    ...
    if (queueLen === 1) {
      if (typeof options.elements[0] === "object" && options.elements[0].nodeName !== "BODY") {
        ...
        ctx.drawImage(canvas, Math.floor(bounds.left), Math.floor(bounds.top), Math.floor(bounds.width), Math.floor(bounds.height), 0, 0, Math.floor(bounds.width), Math.floor(bounds.height));
        canvas = null;
        return newCanvas;
      }
    }
